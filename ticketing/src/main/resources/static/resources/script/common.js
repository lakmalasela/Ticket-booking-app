...
function getLocalDatetime(format,extdatetime=""){
    if(extdate != ""){
        nowdate = new Date(extdatetime);
    }else{
        nowdate = new Date();
    }
...
function getCurrentDateTime(format){
    ...
    var today = new Date();
    var month = today.getMonth()+1;//month ekea enne array ekakin(0-11)
    if(month<10) month = "0"+month;
    //date eka enne range ekakin (1-31)
...
function getAfteryearDateTime(format){
...
function listCompera(list1,list2,atb1,atb2) {
    ...
    list1 = list1.filter(function(o1){
        ...
        return !list2.some(function(o2){//atleast one elemet satis the condition
            return o1.id === o2.id;          // assumes unique id
        });
    }).map(function(o){//map make the new array
...
function table(parentid,id,metadata, hasBtn=true){
    ...
    thead.classList.add('theadcolor');
    ...
    for(var i=1; i<=metadata.length; i++){
        ...
        th.innerHTML = metadata[i-1]['name'] ;
            // +
            // "   "
            // + '<i class="fa fa-sort isort" aria-hidden="true"></i>';
        row1.appendChild(th);
        var th = createElement('th');
...
function fillTable(tableid,obs,modifyFunction=null,deletefunction=null,printfunction=null){
    ...
    for(var i=0; i<obs.length; i++){
        ...
        if(hasButton){
            ...
            td.classList.add('btnSize');//my added
            ...
            buttonup.classList.add('btnDelUpsVieize');//myadded
            buttonup.classList.add('editbtn');
            ...
            buttondel.classList.add('btnDelUpsVieize');//myadded
            buttondel.classList.add('deletebtn');
            ...
            buttonvie.classList.add('btnDelUpsVieize');//myaddded
            buttonvie.classList.add('viewbtn');
...
function textFieldBinder(field,pattern,obj,prop,oldobj){
    ...
    var oldob = window[field.getAttribute('data-oldobject')];//attribute ekee value eka gnnwa
...
function CheckBoxBinder(field,pattern,obj,prop,oldobj){
    field.setAttribute('data-object',obj); //set the new attribute data-object
    field.setAttribute('data-oldobject',oldobj);
    var ob = window[field.getAttribute('data-object')];
